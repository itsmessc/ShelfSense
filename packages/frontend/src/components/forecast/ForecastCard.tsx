...
import { CONFIDENCE_COLOURS } from '../../constants/index.js';
...
function ProviderBadge({ forecast }: { forecast: ForecastResult }) {
  if (forecast.ai_provider === 'gemini') {
    return (
  ...
  if (forecast.ai_provider === 'openai') {
    return (
  ...
  return (
    <div className="bg-white rounded-2xl shadow-sm border p-6 space-y-4">
      <div className="flex items-center justify-between">
        ...
        <ProviderBadge forecast={forecast} />
      </div>
      ...
      {forecast.days_until_stockout != null ? (
        <div className="grid grid-cols-2 gap-4">
          ...
          <div className="bg-gray-50 rounded-xl p-3 text-center">
            <p className={`text-lg font-bold ${CONFIDENCE_COLOURS[forecast.confidence]}`}>{pct}%</p>
      ...
      {!forecast.ai_generated && (
        <p className="text-xs text-amber-600">
          Using rule-based average calculation
        </p>
      )}
    </div>
  );
